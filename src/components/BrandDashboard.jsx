          ...
          <div className="flex items-center gap-6 ml-8">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-zinc-200">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold">Nike Marketing</p>
    ...
    >
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2" style={{ color: BLACK }}>
            <DollarSign size={20} />
            <span className="text-sm font-bold uppercase tracking-wider opacity-80">Agency Commissions Saved</span>
          </div>
          <motion.h2 
            key={savingsAmount}
          ...
          <p className="text-sm opacity-70" style={{ color: BLACK }}>Based on standard 20% agency fees</p>
        </div>
      </motion.div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold">Campaign Pulse</h2>
          <button className="text-sm font-bold text-zinc-900 hover:underline flex items-center gap-1">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-zinc-500">Powered by Lumi AI</span>
        </div>
...
function CampaignCard({ campaign, index }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-sm hover:shadow-xl transition-all cursor-pointer group"
    >
      ...
      <h3 className="font-bold text-lg mb-4 line-clamp-2">{campaign.name}</h3>
      ...
      <div className="flex items-center gap-6 mb-6">
        <div className="relative w-20 h-20">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-xl font-bold" style={{ color: progressColor }}>{campaign.progress}%</span>
          </div>
        </div>
...
function PipelineStage({ stage, index }) {
  ...
  return (
    <motion.div
      ...
      className={cn("rounded-xl border-2 p-4 text-center", colors[stage.color])}
    >
      <p className="text-3xl font-bold mb-1">{stage.count}</p>
      <p className="text-xs font-bold uppercase tracking-wider">{stage.name}</p>
    </motion.div>
  );
}
...
function UrgentActionCard({ action, index }) {
  const typeIcons = {
    approval: <AlertCircle size={20} className="text-rose-500" />,
    review: <FileText size={20} className="text-amber-500" />,
    notification: <CheckCircle size={20} className="text-blue-500" />,
  };
  ...
  return (
    <motion.div
      ...
      transition={{ delay: index * 0.1 }}
      className="bg-white rounded-xl border border-zinc-200 p-4 flex items-center justify-between hover:border-zinc-300 transition-colors group"
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="font-bold text-zinc-900">{action.creator}</p>
          <p className="text-sm text-zinc-500">{action.action}</p>
          <p className="text-xs text-zinc-400 mt-0.5">{action.campaign}</p>
        </div>
      </div>
      
      <div className="flex items-center gap-3">
        {action.cost > 0 && (
          <span className="text-sm font-bold text-zinc-900">+${action.cost}</span>
        )}
        <motion.button
